  ...
  return (
    <div className="w-screen pb-10 h-auto font-sans">
      <div className="container mx-auto">
        <div className="flex flex-wrap sm:px-10 mt-3">
        ...
        <div className="my-4 px-auto grid grid-cols-3 xl:grid-cols-6 text-center xl:text-start">
          {['Top Material', 'Leg Material', 'Top Finish', 'Leg Finish', 'Category', 'Knocked Down'].map((label) => (
            <div key={label} className="relative flex justify-center">
              ...
              {/* Dropdown */}
              {openDropdown === label && (
                <div className="absolute right-0 mt-10 w-24 z-10 sm:w-48 bg-white border border-gray-200 rounded-md shadow-lg">
        ...
        <div className="sm:pr-9 sm:pl-12">
          <div className=" grid gap-4 md:gap-6 grid-cols-2 ">
            {images.map((image) => (
              <Link key={image.id} href="/productdetails" className="flex flex-col items-center justify-center border p-1 md:p-3 h-36 sm:h-44 md:h-56 lg:h-64 xl:h-72 2xl:h-80 text-center hover:scale-105 transition-transform" >
